...
function summaryHTML(){const a=LAYOUTS[option][width].audit,critical=a.issues.filter(i=>!i.hard&&i.gap<1.5),o=M.options[option],inv=inventory();
 ...
 return `<span class="eyebrow">Option ${{u:'A',wide:'B',rounds:'C',d:'D'}[option]}</span><h3>${esc(o.name)}</h3><p>${esc(o.trade)}</p><dl><dt>Head-table seats</dt><dd>25</dd><dt>Head-table 6-ft sections</dt><dd>${o.six}</dd><dt>Guest rounds</dt><dd>${inv.rounds}</dd><dt>Single 6-ft guest tables</dt><dd>${inv.small}</dd><dt>Joined guest tables</dt><dd>${inv.long} × 2 sections</dd><dt>Total dinner 6-ft sections</dt><dd>${inv.six}</dd></dl><p class="caption">${caption}</p><div class="status ${a.hard||critical.length?'bad':''}">${status}</div><p class="route-status"><strong>${a.routeClear?'Kitchen, entry and boba spaces are clear in the model.':'A kitchen, entry or boba space has a conflict.'}</strong> Keep the blue areas free of chairs, displays and queues.</p><details><summary>Closest areas to check</summary><ol class="fit-list">${issues.map(i=>`<li>${esc(label(i.a))} / ${esc(label(i.b))}: ${i.gap<0?Math.abs(i.gap).toFixed(1)+'-ft overlap':i.gap.toFixed(1)+'-ft gap'}</li>`).join('')}</ol></details>${roster}`;
}
function render(){
 $('choices').innerHTML=Object.entries(M.options).map(([id,o])=>{const inv=inventory(id),a=LAYOUTS[id][width].audit;return `<button type="button" class="choice ${id===option?'active':''}" data-option="${id}" aria-pressed="${id===option}"><span class="tag">${id==='u'?'Compact U · fits, but very tight':id==='wide'?'Longer arms · DOES NOT FIT at true scale':id==='d'?'Compact U · parents into the open lane':'Straight head · best fit'}</span><strong>${esc(o.name)}</strong><small>${o.six} head sections · ${inv.rounds} rounds · ${inv.long} joined · ${a.hard?a.hard+' overlaps':'min gap '+a.minGap.toFixed(1)+' ft'}</small></button>`;}).join('');
 ...
 $('headTrade').textContent=M.compactU(option)?'Compact U: 12 face the band, 11 sit side-on and 2 end seats face away. One arm has 6 people and the other 7, using both long sides plus an end seat. The table supports and corner place settings need checking.'+(option==='d'?' In option D the parents\u2019 Tables 1 and 7 may sit into the keep-open lane, flanking the view corridor.':''):M.usesU(option)?'The U keeps 12 guests facing the band and 13 side-on. Nobody sits directly with their back to the stage. Eric and Meg remain together in the centre; Andrew is beside Eric and Mabelle beside Meg.':'Twelve guests face the band; twelve in the opposite row have their backs toward it, and Oliver sits at the end. This uses four fewer head-table sections than the longer-arm U.';
 renderTables();
}
