...
function tableCard(t){
 ...
 let note=t.id==='t6'?'Sam and Adelia sit side by side. Danielle and Matt sit across from each other.':t.id==='t11'?'Seven-person round. Keep Tables 11 and 13 nearby.':t.id==='t13'?'Six guests at one 6-ft table. Preserve the displayed seat order and keep Table 11 nearby.':t.id==='t10'?'Ten guests at two joined 6-ft sections.':'';
 if(!pos)note='NOT PLACED IN OPTION C: this group\u2019s '+t.guests.length+' guests fold into open round seats. Regrouping with Meg is pending; the guest list and meals stay as shown.';
 else if(option==='rounds'&&M.mix.has(t.id))note=(note?note+' ':'')+'Option C seats this group at a 60-inch round.';
...
function summaryHTML(){const a=LAYOUTS[option][width].audit,critical=a.issues.filter(i=>!i.hard&&i.gap<1.5),o=M.options[option],inv=inventory();
 ...
 const caption=option==='rounds'?'Tables 6 and 10 each seat ten at two joined sections; every other guest table is a 60-inch round (max 8). Table 13\u2019s six guests regroup into open round seats — assignments pending with Meg. Display furniture and the cake table are additional.':'Tables 6 and 10 each seat ten at two joined sections. Table 11 is a seven-person round; Table 13 seats six at one section. Display furniture and the cake table are additional.';
 return `<span class="eyebrow">Option ${{u:'A',wide:'B',rounds:'C'}[option]}</span><h3>${esc(o.name)}</h3><p>${esc(o.trade)}</p><dl><dt>Head-table seats</dt><dd>25</dd><dt>Head-table 6-ft sections</dt><dd>${o.six}</dd><dt>Guest rounds</dt><dd>${inv.rounds}</dd><dt>Single 6-ft guest tables</dt><dd>${inv.small}</dd><dt>Joined guest tables</dt><dd>${inv.long} × 2 sections</dd><dt>Total dinner 6-ft sections</dt><dd>${inv.six}</dd></dl><p class="caption">${caption}</p><div class="status ${a.hard||critical.length?'bad':''}">${status}</div><p class="route-status"><strong>${a.routeClear?'Kitchen, entry and boba spaces are clear in the model.':'A kitchen, entry or boba space has a conflict.'}</strong> Keep the blue areas free of chairs, displays and queues.</p><details><summary>Closest areas to check</summary><ol class="fit-list">${issues.map(i=>`<li>${esc(label(i.a))} / ${esc(label(i.b))}: ${i.gap<0?Math.abs(i.gap).toFixed(1)+'-ft overlap':i.gap.toFixed(1)+'-ft gap'}</li>`).join('')}</ol></details>${roster}`;
}
function render(){
 $('choices').innerHTML=Object.entries(M.options).map(([id,o])=>{const inv=inventory(id);return `<button type="button" class="choice ${id===option?'active':''}" data-option="${id}" aria-pressed="${id===option}"><span class="tag">${id==='u'?'U shape · keeps all current groups':id==='wide'?'Longer arms · no backs to band':'Mirrored rounds · regrouping needed'}</span><strong>${esc(o.name)}</strong><small>${o.six} head sections · ${inv.rounds} rounds · ${inv.small} small · ${inv.long} joined</small></button>`;}).join('');
