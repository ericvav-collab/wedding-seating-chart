...
function render(){
 $('choices').innerHTML=Object.entries(M.options).map(([id,o])=>{const inv=inventory(id),a=LAYOUTS[id][width].audit;return `<button type="button" class="choice ${id===option?'active':''}" data-option="${id}" aria-pressed="${id===option}"><span class="tag">${id==='u'?'Compact U · fits, but very tight':id==='wide'?'Longer arms · DOES NOT FIT at true scale':'Straight head · best fit'}</span><strong>${esc(o.name)}</strong><small>${o.six} head sections · ${inv.rounds} rounds · ${inv.long} joined · ${a.hard?a.hard+' overlaps':'min gap '+a.minGap.toFixed(1)+' ft'}</small></button>`;}).join('');
