...
let DATA,LAYOUTS,option='fay',selected='head',space=true,venueView='all';
...
function venueSVG(){
 ...
 for(const a of VenuePlan.stations){
  ...
  if(large)s+=tx(a.x+a.w/2,a.y+a.h/2,a.title,11)+tx(a.x+a.w/2,a.y+a.h/2+17,a.id==='booth'?'8 × 8 ft':'COCKTAIL HOUR',9);
...
function tableCard(t){
 ...
 let note=t.id==='t6'?'Sam and Adelia sit side by side. Danielle and Matt sit across from each other. Only joined table in the room (A&M group of ten).':t.id==='t14'?'New table from Meg\u2019s Sep 20 regrouping (former Table 10 long-table guests).':'';
...
function summaryHTML(){const a=LAYOUTS[option][width].audit,critical=a.issues.filter(i=>!i.hard&&i.gap<1.5),o=M.options[option],inv=inventory();
 let status=option==='fay'?`<strong>Milano’s own drawing fits this furniture.</strong> This site’s room is an approximate 50 × 61-ft working footprint, so it shows ${critical.length} gaps under 1.5 ft that the real room does not have. Use Fay’s Sep 25 floorplan for exact table placement; use this page for who sits where.`:a.hard?`<strong>${a.hard} overlaps in this placement.</strong> Revise furniture or placement within the fixed room.`:critical.length?`<strong>Too tight for setup as drawn.</strong> ${critical.length} ${critical.length===1?'gap is':'gaps are'} below 1.5 ft between occupied spaces; the closest guest-table gap is ${a.minGap.toFixed(2)} ft.`:`<strong>No occupied-space overlaps.</strong> ${a.tight?a.tight+' gaps are below the 3-ft circulation target.':'All checked gaps meet the 3-ft model target.'} Confirm the actual dimensions with Milano.`;
 ...
 const caption='Table 6 (A&M group of ten) is the only joined table; all 13 other guest tables are 60-inch rounds seating up to 8, per Meg\u2019s Sep 20 chart. Display furniture and the cake table are additional.';
 return `<span class="eyebrow">${option==='fay'?'Selected · Milano\u2019s floorplan':'Earlier option '+{u:'A',wide:'B',rounds:'C',d:'D'}[option]}</span><h3>${esc(o.name)}</h3><p>${esc(o.trade)}</p><dl><dt>Head-table seats</dt><dd>25</dd><dt>Head-table 6-ft sections</dt><dd>${o.six}</dd><dt>Guest rounds</dt><dd>${inv.rounds}</dd><dt>Single 6-ft guest tables</dt><dd>${inv.small}</dd><dt>Joined guest tables</dt><dd>${inv.long} × 2 sections</dd><dt>Total dinner 6-ft sections</dt><dd>${inv.six}</dd></dl><p class="caption">${caption}</p><div class="status ${option!=='fay'&&(a.hard||critical.length)?'bad':''}">${status}</div><p class="route-status"><strong>${a.routeClear?'Kitchen, entry and boba spaces are clear in the model.':'A kitchen, entry or boba space has a conflict.'}</strong> Keep the blue areas free of chairs, displays and queues.</p><details><summary>Closest areas to check</summary><ol class="fit-list">${issues.map(i=>`<li>${esc(label(i.a))} / ${esc(label(i.b))}: ${i.gap<0?Math.abs(i.gap).toFixed(1)+'-ft overlap':i.gap.toFixed(1)+'-ft gap'}</li>`).join('')}</ol></details>${roster}`;
}
function render(){
 $('choices').innerHTML=Object.entries(M.options).map(([id,o])=>{const inv=inventory(id),a=LAYOUTS[id][width].audit;return `<button type="button" class="choice ${id===option?'active':''}" data-option="${id}" aria-pressed="${id===option}"><span class="tag">${id==='fay'?'SELECTED · Milano\u2019s Sep 25 floorplan':id==='u'?'Earlier · compact U, very tight':id==='wide'?'Earlier · longer arms, both sides seated':id==='d'?'Earlier · parents into the open lane':'Earlier · straight head'}</span><strong>${esc(o.name)}</strong><small>${o.six} head sections · ${inv.rounds} rounds · ${inv.long} joined · ${a.hard?a.hard+' overlaps':'min gap '+a.minGap.toFixed(1)+' ft'}</small></button>`;}).join('');
 ...
 $('headTrade').textContent=M.outsideOnly(option)?'Milano\u2019s plan: all 25 seats sit on the outside of the U, so every head-table guest faces the dance floor and band. Eric and Meg stay centred on the wall row with Andrew and Mabelle beside them; Meg\u2019s arm seats 7 (one table of 4) and Eric\u2019s arm seats 6.':M.compactU(option)?'Compact U: 12 face the band, 11 sit side-on and 2 end seats face away. One arm has 6 people and the other 7, using both long sides plus an end seat. The table supports and corner place settings need checking.'+(option==='d'?' In option D the parents\u2019 Tables 1 and 7 may sit into the keep-open lane, flanking the view corridor.':''):M.usesU(option)?'The U keeps 12 guests facing the band and 13 side-on. Nobody sits directly with their back to the stage. Eric and Meg remain together in the centre; Andrew is beside Eric and Mabelle beside Meg.':'Twelve guests face the band; twelve in the opposite row have their backs toward it, and Oliver sits at the end. This uses four fewer head-table sections than the longer-arm U.';
 renderTables();
}
