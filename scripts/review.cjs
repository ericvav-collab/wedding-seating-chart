...
'',
'THREE OPTIONS AT CONFIRMED TABLE SCALE (Fay, Sep 9: 60-inch rounds,',
'6-ft rectangles — the only sizes Milano has)',
'A: Compact U + mixed guest tables. 6 head sections (4 back + 1 per arm);',
'   7 rounds + 4 single 6-ft + 2 joined 12-ft guest tables; 14 dinner 6-ft sections.',
'   12 head seats face the band, 11 side-on, 2 end seats face away.',
'   All current guest groups keep their tables.',
'B: Longer-arm U + mixed guest tables. 8 head sections (4 back + 2 per arm);',
'   same guest tables as A; 16 dinner 6-ft sections.',
'   12 head seats face the band, 13 side-on. Nobody faces away.',
'C: Symmetrical rounds + straight head table. 4 head sections;',
'   ten 60-inch rounds (five per side, mirrored) + 2 joined 12-ft guest',
'   tables; 8 dinner 6-ft sections — the smallest furniture order.',
'   12 head seats face the band, 12 face away, 1 end seat.',
'   REGROUPING REQUIRED: Meg\u2019s round groups plus Table 13\u2019s six guests',
'   must reform into five tables of eight; assignments pending with Meg.',
'   Boba rotates along the rear Monza wall inside the lobby entry.',
'Rounds seat up to 8. Rectangular guest tables use long-side seating.',
'Tables 6 and 10 each seat 10 at two joined 6-ft sections in every option.',
...
lines.push('GUEST SEATS — GROUPS AS IN OPTIONS A/B','(In option C every guest table except 6 and 10 is a 60-inch round,','and Table 13\u2019s guests regroup into open round seats.)','');
